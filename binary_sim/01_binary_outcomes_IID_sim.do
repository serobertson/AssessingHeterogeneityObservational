...
import excel using "marg_cond_binary_rd.xlsx" , clear first
...
local dif_rd = `=diff_RD[`scenario']'

local sub1_p1cond = 	`=sub1_p1_conditional[`scenario']'
local sub1_p0cond = 	`=sub1_p0_conditional[`scenario']'
local sub0_p1cond = 	`=sub0_p1_conditional[`scenario']'
local sub0_p0cond = 	`=sub0_p0_conditional[`scenario']'	
...
postfile `saved_results' double(scenario sim_run N_pt prev sub1_rd sub0_rd dif_rd  ///
								...
								binary_Y_IID_rd_sim`scenario'_`N_pt'_`prev'.dta, replace 
...
generate Y = runiform() < invlogit( `sub1_p1cond' + log(1.5) * X1 + log(3) * X2 + log(2) * X3 + log(3) * X4   ///
												+ log(3) * X5 + log(2) * X6 + log(3) * X7 + log(2) * X8 ) if S == 1 & A==1
												
replace Y =  runiform() < invlogit( `sub1_p0cond' + log(1.5) * X1 + log(3) * X2 + log(2) * X3 + log(3) * X4   ///
												+ log(3) * X5 + log(2) * X6 + log(3) * X7 + log(2) * X8 ) if S == 1 & A==0	
												
replace Y =  runiform() < invlogit( `sub0_p1cond' + log(3) * X1 + log(3) * X2 + log(2) * X3 + log(3) * X4   ///
												+ log(3) * X5 + log(2) * X6 + log(3) * X7 + log(2) * X8 ) if S == 0 & A==1	 
												
replace Y =  runiform() < invlogit( `sub0_p0cond' + log(3) * X1 + log(3) * X2 + log(2) * X3 + log(3) * X4   ///
											+ log(3) * X5 + log(2) * X6 + log(3) * X7 + log(2) * X8 ) if S == 0 & A==0																					
...
if `error_check1' == 0 | full_rank==1  {
	...
	generate g_OM_A1 = 		invlogit(estimatesOM[1,1] * X1 ///
								+ estimatesOM[1,2] * X2 ///
								+ estimatesOM[1,3] * X3 ///
								+ estimatesOM[1,4] * X4 ///
								+ estimatesOM[1,5] * X5 ///
								...
								+ estimatesOM[1,13])
	generate g_OM_A0 = 		invlogit(estimatesOM[1,1] * X1 ///
								+ estimatesOM[1,2] * X2 ///
								+ estimatesOM[1,3] * X3 ///
								+ estimatesOM[1,4] * X4 ///
								+ estimatesOM[1,5] * X5 ///
...
capture logit Y  X1 X2 X3 X4 X5 X6 X7 X8 A inter_A_S S interX1_S [pw = w1]
local error_check1 = _rc
...
if `error_check1' == 0 | full_rank==1 {
	...
	generate g_DR_A1= 		invlogit(estimates_DR[1,1] * X1 ///
								...
								+ estimates_DR[1,13]) 
	generate g_DR_A0 = 	invlogit(estimates_DR[1,1] * X1 ///
								+ estimates_DR[1,2] * X2 ///
								+ estimates_DR[1,3] * X3 ///
								+ estimates_DR[1,4] * X4 ///
								+ estimates_DR[1,5] * X5 ///
